...
CREATE OR REPLACE TRIGGER trigger_upt_dt_location
BEFORE UPDATE ON "&schema_name"."LOCATION"
FOR EACH ROW 
BEGIN
...
ALTER TRIGGER trigger_upt_dt_location DISABLE;
--si se poate activa la loc 
ALTER TRIGGER trigger_upt_dt_location ENABLE;
-- acum vom crea o secventa
DROP SEQUENCE sequence_location_id;
CREATE SEQUENCE sequence_location_id 
START WITH 1
INCREMENT BY 1
NOCACHE
NOCYCLE;
-- acum vom crea un trigger care va prelua ultima valoare din secventa de mai sus si o va face cheie primara pentru 
-- inregistrarile pe care incercam sa le introduce in tabela LOCATION.
CREATE OR REPLACE TRIGGER trigger_seq_location_id_pk
BEFORE INSERT ON "&schema_name"."LOCATION"
FOR EACH ROW
BEGIN
    :NEW.LOCATION_id := sequence_location_id.NEXTVAL;
END;
/
--trigger-ul odata rulat este activat
--acesta se poate dezactiva folosind comanda
ALTER TRIGGER trigger_seq_location_id_pk DISABLE;
--si se poate activa la loc 
ALTER TRIGGER trigger_seq_location_id_pk ENABLE;
COMMIT;